...
class Dream_run_setup(object):
    def __init__(self):
        ...
        self.params = [spotpy.parameter.Uniform('Lateral_Conductivity_61',low=0.00001, high=0.01,  optguess=0.001),
    ...
    def simulation(self,x):
        ...
        change_setting(config_file, "Exponential Decrease 61", str(round(x[1],4)))
        change_setting(config_file, "Precipitation Lapse Rate", str(round(x[2],4)))
        change_setting(config_file, "Temperature Lapse Rate", str(round(x[3],4)))
#         change_setting(config_file, "Overstory Monthly LAI    9"," ".join([str(round(x[3],3))]*12))
        ...
        Simulation_streamflow = Simulation_streamflow[7:-14]
